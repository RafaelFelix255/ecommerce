...
$app->post("/profile", function(){
	...
	if ($_POST['desemail'] !== $user->getdesemail()){
		if (User::checkLoginExist($_POST['desemail']) === true){
			User::setError("Este endereço de e-mail já está sendo utilizado por outro usuário.");
			header("Location: /profile");
			exit;
		}
	}
	...
	User::setSuccess("Suas informações foram alteradas com sucesso!");
...
$app->get("/boleto/:idorder", function($idorder){
	...
	$dadosboleto["endereco2"] = $order->getdesdistrict() . " - " . $order->getdesstate() . " - CEP: " . $order->getdeszipcode();
...
$app->post("/profile/change-password", function(){
	...
	if (!password_verify($_POST['current_pass'], $user->getdespassword())){
		User::setError("A senha digitada está inválida.");
		header("Location: /profile/change-password");
		exit;
	}
